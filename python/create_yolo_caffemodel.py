...
def main(argv):
	...
	for pr in params:
		...
		if layer.type == 'Convolution':
			...
			print("bias=%d,bn=%d,wgt=%d,count=%d"%(biasSize,bnSize,weightSize,count))
			...
			print (pr+"(dconv)" + "-"+str(layercnt)+"-"+str(len(net.params[pr]) > 1))
            # # bias
			# print(len(net.params[pr]))
			# if len(net.params[pr]) > 1:
			# 	bias_dim = net.params[pr][1].data.shape
			...
			# net.params[pr][0].data[...] = np.reshape(netWeights[count:count+weightSize], dims)
			# count = count + weightSize
			# print("bias=%d,wgt=%d,count=%d"%(biasSize,weightSize,count))
